#!/usr/bin/env python3
"""
CORRECTED Preprocessing script for RML2016.10a dataset
Implements proper 6:3:1 split as specified in the MAC paper
Preserves exact data format from data201610a.py
"""
import pickle
import numpy as np
import torch
...
def create_631_split_rml2016a():
    """
    Create proper 6:3:1 splits for RML2016.10a dataset
    Following the exact logic from data201610a.py but with correct ratios
    """
    print("=" * 60)
    print("CREATING CORRECTED 6:3:1 SPLITS FOR RML2016.10a")
    print("=" * 60)
    
    # Load raw data
    filename = 'data/raw/RML2016.10a_dict.pkl'
    ...
    for mod in mods:
        for snr in snrs:
            ...
            # CORRECTED: Create 6:3:1 split instead of 8:2
            # For each (mod, snr) combination with 1000 samples:
            # - 600 samples for training (60%)
            # - 300 samples for validation (30%) 
            # - 100 samples for testing (10%)
            
            total_range = range(a * 1000, (a + 1) * 1000)
            train_samples = list(np.random.choice(total_range, size=600, replace=False))
            remaining_samples = list(set(total_range) - set(train_samples))
            val_samples = list(np.random.choice(remaining_samples, size=300, replace=False))
    ...
    for snr in tqdm(snrs, desc="Processing SNR levels"):
        ...
        # Save with original naming convention for compatibility
        save_pickle(snr_train_dataset, f'data/processed/RML2016.10a/{snr}_train_MV_dataset')
        save_pickle(snr_val_dataset, f'data/processed/RML2016.10a/{snr}_val_MV_dataset')
        save_pickle(snr_test_dataset, f'data/processed/RML2016.10a/{snr}_test_MV_dataset')
    
    # Save ALL SNR datasets
    print("Saving ALL SNR datasets...")
    save_pickle(train_dataset, 'data/processed/RML2016.10a/train_ALL_SNR_MV_dataset')
    save_pickle(val_dataset, 'data/processed/RML2016.10a/val_ALL_SNR_MV_dataset')
    save_pickle(test_dataset, 'data/processed/RML2016.10a/test_ALL_SNR_MV_dataset')
    ...
    save_pickle(split_info, 'data/processed/RML2016.10a/split_info.pkl')
    
    print(f"\n✅ RML2016.10a preprocessing complete!")
    print(f"Generated {len(snrs)} SNR-specific datasets + ALL SNR dataset")
    print(f"Split ratios: Train={split_info['train_ratio']:.3f}, Val={split_info['val_ratio']:.3f}, Test={split_info['test_ratio']:.3f}")
    
    return True

def verify_rml2016a_splits():
    """Verify the generated splits are correct"""
    print("\n" + "=" * 60)
    print("VERIFYING RML2016.10a SPLITS")
    print("=" * 60)
    
    # Load split info
    split_info = load_pickle('data/processed/RML2016.10a/split_info.pkl')
    ...
    try:
        train_all = load_pickle('data/processed/RML2016.10a/train_ALL_SNR_MV_dataset')
        val_all = load_pickle('data/processed/RML2016.10a/val_ALL_SNR_MV_dataset')
        test_all = load_pickle('data/processed/RML2016.10a/test_ALL_SNR_MV_dataset')
        
        print(f"✅ ALL SNR datasets loaded successfully")
        print(f"   Train: {len(train_all)} samples")
        print(f"   Val:   {len(val_all)} samples")
        print(f"   Test:  {len(test_all)} samples")
        
        # Test data format
        sample_data, sample_label, sample_idx = train_all[0]
...
if __name__ == "__main__":
    ...
    success = create_631_split_rml2016a()
    
    if success:
        # Verify the splits
        verify_rml2016a_splits()
    else:
        print("❌ Failed to create RML2016.10a splits")
